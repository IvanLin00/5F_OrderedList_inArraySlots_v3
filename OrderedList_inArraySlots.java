...
public class OrderedList_inArraySlots
    ...
    public int indexOf_whileStyle( Integer findMe) {
		 ...
		 int high = size()-1;
		 while (low <= high){
			 int pageToFlip =(int) ((low + high)/2);
			 if (get(pageToFlip) == findMe) return pageToFlip;
			 else if (get(pageToFlip) < findMe) low = pageToFlip + 1;
			 else high = pageToFlip - 1;
		 }
		 return -1;
    }
    
	public int indexOf_recursive(Integer findMe){
	...
	private int indexOf_recursive(Integer findMe,int low, int high){
		 ...
		 int pageToFlip = (int) ((low+high)/2);
		 if (get(pageToFlip) == findMe) return pageToFlip;
		 if (get(pageToFlip) < findMe) return indexOf_recursive(findMe, pageToFlip + 1, high);
		 else return indexOf_recursive(findMe, low, pageToFlip-1);
	}
